...
var DEBUG_CONSOLE = globalThis.document ? (new URL(globalThis.document.location.href)).searchParams.get('debug') !== null : false;
function debug(...args) {
    if(!DEBUG_CONSOLE) {
        return;
    }
        ...
        case P9_TSTATFS:
            ...
            function countFile() {
                return new Promise((resolve) => {
                    const ctx = fs.provider.openReadWriteContext();
                    const store = ctx._getObjectStore();
            ...
            Promise.all([countFile(), countSize()]).then((result) => {
                const [fileCount, storageInfo] = result;
                ...
                var total_size;
                var space;
                if (storageInfo.usageDetails) {
                    total_size = storageInfo.usageDetails.indexedDB;
                    space = 256 * 1024 * 1024;
                } else {
                    total_size = storageInfo.usage || 0;
                    space = storageInfo.quota || 256 * 1024 * 1024;
                }
                ...
                var f_bavail =
                    f_blocks - Math.floor(total_size / f_bsize); /* Free blocks available to unprivileged user */
                var f_files = fileCount || 0; /* Total file nodes in filesystem */
                ...
                var statfs = [f_type, f_bsize, f_blocks, f_bfree, f_bavail, f_files, f_ffree, f_fsid, f_namelen];
...
*/
        ...
        // https://github.com/copy/v86/blob/d2f85b46d07cbb924ed30a8baf2402bf8bca81d8/lib/9p.js#L821
        // This part of the implementation is currently commented out
        case P9_TXATTRWALK:
            var req = marshall.Unmarshall2(["w", "w", "s"], GetByte);
            var fid = req[0];
            var newfid = req[1];
